...
public abstract class Record implements Cloneable, Comparable, Serializable {
    ...
    newRecord(Name name, int type, int dclass, long ttl, int length, DNSInput in)
        ...
        if (in != null) {
            ...
            if (in.remaining() > 0 && in.remaining() % 4 != 0)
                throw new WireParseException("invalid record length : " +
                        String.format("length=%s remaining=%s",
                                String.valueOf(length),
                                String.valueOf(in.remaining()))
                );
            else in.readByteArray(in.remaining()); // If there are additional bytes consume it.
            in.clearActive();
        }
        return rec;
    }
